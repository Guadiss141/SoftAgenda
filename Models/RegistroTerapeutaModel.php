...
class TerapeutaModel {
    ...
    public function registrarTerapeuta(
        ...
        try {
            ...
            // 2. Insertar en usuario (rol 2 = Terapeuta)
            $id_Rol = 2;
            $sql_usuario = "INSERT INTO usuario (Usuario_Nombre, Correo_E, Contrasena, id_Rol) VALUES (?, ?, ?, ?)";
            ...
            $sql_empleado = "INSERT INTO empleado (id_Persona, id_Usuario, Especialidad, CUIL) VALUES (?, ?, ?, ?)";
            $stmt = $this->conexion->prepare($sql_empleado);
            $stmt->bind_param("iiss", $id_Persona, $id_Usuario, $especialidad, $cuil);
            $stmt->execute();
